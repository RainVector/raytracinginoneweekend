...
vec3 color(ray r, sphere hitableList[2], int num)
{
    ...
    for(int i = 0; i < num; i++)
    {
        hit_record ht = hit_sphere(r,hitableList[i],0.0,closest_so_far);
    	if(ht.isHit)
        {
...
{ 
    ...
    float u = (fragCoord.x + rand(fragCoord))/iResolution.x ;
    float v = (fragCoord.y + rand(fragCoord))/iResolution.y;
    ...
    sphere hitableList[2] = sphere[2](s1,s2);
